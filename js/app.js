...
function buscarImagenes(){
    ...
    const key = '32809450-878ef6c61e28b6dc48341835a';
    ...
    fetch(url)
        ...
        .then(resultado => {
            if(resultado.totalHits !== 0){
                ...
                mostrarAlerta('No hay resultados a su busqueda, pruebe con algo diferente.');
            }
        });
}
...
function imprimirPaginador(){
    ...
    while(true){
        ...
        // caso constrario genera un boton en el generador
        const boton = document.createElement('A');
        boton.href = '#';
        boton.dataset.pagina = value;
        boton.textContent = value;
        if(paginaActual === value){
            boton.classList.add('bg-yellow-100');
        }else{
            boton.classList.add('bg-yellow-400');
        }
        boton.classList.add('siguiente', 'px-4', 'py-1', 'mr-2', 'font-bold', 'mb-10', 'rounded');
        boton.onclick = () => {
            paginaActual = value;
            buscarImagenes();
        }
